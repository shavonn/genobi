...
{{/if}}`,
  ...
  /* Add your styles here */
    ...
    it("should reject a generator with conflicting operation options", () => {
      expect(() => {
        // @ts-expect-error should be missing prompts for validation
  ...
  describe("Partial validation scenarios", async () => {
    ...
    it("should accept valid partials", () => {
      expect(() => {
        ...
        api.addPartial(
          "list",
          (context: any) => `<ul>${context.items.map((item: any) => `<li>${item}</li>`).join("")}</ul>`,
        );
      }).not.toThrow();
    });
          ...
          operations: [
            {
              ...
              items: [
                { name: "file1", content: "Content 1" },
                { name: "file2", content: "Content 2" },
              ],
            },
          ],
        });
